...
HH3:=0;
	...
	ELSIF (C='Khulna') THEN
		select availablePos into HH from Employer@site where lower(nameEm)=lower(A) and email=D and address=C and lower(requirement)=lower(B);
		HH3:=personCheck(E);
		if HH>0 and HH3=1 THEN
			UPDATE Employer@site SET availablePos = HH-1 WHERE lower(nameEm)=lower(A) and email=D and address=C and lower(requirement)=lower(B);
			insert into applied@site values(HH2,A,B,D,E,F);
			HH2:=HH2+1;
			DBMS_OUTPUT.PUT_LINE('Applied');
		end if;
