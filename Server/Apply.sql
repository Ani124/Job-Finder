...
prompt Enter 1 for apply to look Job inside Dhaka  
prompt Enter 2 for apply to look Job inside Khulna 
...
BEGIN 
	...
	ELSIF a=2 THEN
		DBMS_OUTPUT.PUT_LINE('Company_Name'||'    '||'Address'||'   '||'Requirements'||'   '||'Available_Position');
		FOR R IN (SELECT * FROM Employer@site where availablePos!=0) LOOP
			DBMS_OUTPUT.PUT_LINE(R.nameEm||'   '||R.address||'   '||R.requirement||'   '||R.availablePos);		
		END LOOP;	
